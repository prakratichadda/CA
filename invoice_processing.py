...
def process_invoices(uploaded_file):
    """
    Main function to orchestrate invoice processing, including data loading,
    cleaning, segmentation, fraud detection, entity extraction, and budget analysis.

    Args:
        uploaded_file: A Streamlit UploadedFile object (or file-like object).

    Returns:
        tuple: A collection of dataframes and plotly figures representing the analysis results.
    """

    # Helper functions (nested as per your provided code, but will be called directly)
    def load_and_clean_data(file_obj):
        df = pd.read_csv(file_obj)
        df.columns = df.columns.str.strip().str.lower().str.replace(" ", "_")
        df.drop_duplicates(inplace=True)
        # Ensure critical columns exist before dropping NaNs
        ...
        if 'qty' in df.columns:
            ...
            df['qty'] = 1 # Default quantity if column is missing
            
        if 'job' in df.columns:
            df['job'] = df['job'].fillna("Unknown")
        else:
            df['job'] = "Unknown" # Default job if column is missing
        
        if 'email' in df.columns:
            df['email'] = df['email'].fillna("no-email@unknown.com")
        else:
            df['email'] = "no-email@unknown.com" # Default email if column is missing


        df['invoice_date'] = pd.to_datetime(df['invoice_date'], errors='coerce')
        df['amount'] = pd.to_numeric(df['amount'], errors='coerce')
        if 'qty' in df.columns: # Re-check after fillna
            df['qty'] = pd.to_numeric(df['qty'], errors='coerce')
        
        # Recalculate total_value after ensuring amount and qty are numeric
        df['total_value'] = df['amount'] * df['qty']
        
        df['invoice_month'] = df['invoice_date'].dt.to_period('M').astype(str)

        df = df.dropna(subset=['invoice_date', 'amount', 'total_value']) # Ensure these core columns are clean
        df = df[df['amount'] > 0] # Amounts must be positive for valid invoices
    ...
    def customer_segmentation_analysis(df):
        ...
        if 'city' not in df.columns: df['city'] = 'Unknown City'
        if 'job' not in df.columns: df['job'] = 'Unknown Job'
        ...
        # Sort monthly_revenue by actual date period for correct chronological order
    ...
    def detect_fraud(df):
        ...
        # Rule 1: Amount <= 0 (already handled in load_and_clean_data, but good to keep as a flag)
        df.loc[df['amount'] <= 0, 'fraud_flag_rule'] = 1
        
        # Rule 2: Duplicate invoices based on date, first_name, product_id, amount
        # Add 'amount' to subset for more robust duplicate detection
        duplicates = df.duplicated(subset=['invoice_date', 'first_name', 'product_id', 'amount'], keep=False)
        df.loc[duplicates, 'fraud_flag_rule'] = 1

        # ML-based fraud detection: IsolationForest
        # Ensure 'amount' column is valid for IsolationForest
        features = df[['amount']].copy().dropna()
        if features.empty:
            df['fraud_flag_ml'] = 0
        else:
            model = IsolationForest(n_estimators=100, contamination=0.02, random_state=42)
            # Use original index to align predictions
            df.loc[features.index, 'fraud_flag_ml'] = model.fit_predict(features)
            df['fraud_flag_ml'] = df['fraud_flag_ml'].map({1: 0, -1: 1}) # Map 1 to 0 (normal) and -1 to 1 (anomaly)
            df['fraud_flag_ml'] = df['fraud_flag_ml'].fillna(0).astype(int) # Fill NaNs (if any from features.dropna())

        df['fraud_suspected'] = df[['fraud_flag_rule', 'fraud_flag_ml']].max(axis=1)

        # Ensure 'first_name' exists for display
        if 'first_name' not in df.columns:
            df['first_name'] = 'Unknown Client'
    ...
    def extract_named_entities(df):
        # Ensure all columns used exist, fill with default if not
        cols_to_check = ['first_name', 'last_name', 'invoice_date', 'email', 'city', 'amount', 'product_id', 'job']
        for col in cols_to_check:
            if col not in df.columns:
                df[col] = np.nan # Or a default string like 'N/A'
    ...
    def budget_vs_actual_analysis(df):
        ...
        # Ensure 'job' column exists before grouping
        if 'job' not in df.columns:
            df['job'] = 'Uncategorized' # Default if job column is missing

        df.dropna(subset=["amount", "invoice_date", "job"], inplace=True)
        
        if df.empty:
            return pd.DataFrame(), pd.DataFrame() # Return empty if no valid data after cleaning

        budget_reference = df.groupby("job")["amount"].sum().reset_index()
        budget_reference.rename(columns={"amount": "actual"}, inplace=True)

        np.random.seed(42)
        # Ensure budget generation matches the length of budget_reference
        if not budget_reference.empty:
            ...
            budget_reference["budget"] = 0 # If empty, set budget to 0
        ...
        audit_flags = pd.DataFrame() # Initialize empty
        
        # Duplicates check for audit flags
        # Ensure columns exist before using them in subset
        cols_for_duplicates = ['invoice_date', 'email', 'amount']
        existing_cols = [col for col in cols_for_duplicates if col in df.columns]
        if len(existing_cols) == len(cols_for_duplicates): # Only run if all columns are present
        ...
        # High value invoices check for audit flags
        ...
        # Select relevant columns for audit_flags display
        display_cols = [col for col in ['invoice_id', 'invoice_date', 'first_name', 'amount', 'product_id'] if col in audit_flags.columns]
        if not display_cols and not audit_flags.empty: # Fallback if specific display cols are missing
             audit_flags = audit_flags.head() # Just show a few rows if we can't be specific
        elif display_cols:
